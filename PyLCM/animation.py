...
def animation_init(dt, nt,rm_spec, qa_ts, qc_ts, qr_ts, na_ts, nc_ts, nr_ts, T_parcel_array, RH_parcel_array, q_parcel_array, z_parcel_array):
    ...
    # Convert to FigureWidget for in-place updates (avoids memory leak from repeated fig.show())
...
def animation_call(fig, time_array, t, dt, nt,rm_spec, qa_ts, qc_ts, qr_ts, na_ts, nc_ts, nr_ts, T_parcel_array, RH_parcel_array, q_parcel_array, z_parcel_array):
    ...
    # Uses FigureWidget batch_update for efficient in-place updates (no memory leak)

    with fig.batch_update():
        # Update traces (lines) in all subplots
        fig.data[0].x = time_array[0:t]
        fig.data[0].y = RH_parcel_array[0:t]*100
        fig.data[1].x = time_array[0:t]
        fig.data[1].y = q_parcel_array[0:t]*1e3
        fig.data[2].x = time_array[0:t]
        fig.data[2].y = z_parcel_array[0:t]
        fig.data[3].x = time_array[0:t]
        fig.data[3].y = T_parcel_array[0:t]
        # Update traces for the mixing ratios plot
        fig.data[4].x = time_array[0:t]
        fig.data[4].y = qa_ts[0:t]*1e3
        fig.data[5].x = time_array[0:t]
        fig.data[5].y = qc_ts[0:t]*1e3
        fig.data[6].x = time_array[0:t]
        fig.data[6].y = qr_ts[0:t]*1e3
        # Update traces for the number concentrations plot
        fig.data[7].x = time_array[0:t]
        fig.data[7].y = na_ts[0:t]/1e6
        fig.data[8].x = time_array[0:t]
        fig.data[8].y = nc_ts[0:t]/1e6
        fig.data[9].x = time_array[0:t]
        fig.data[9].y = nr_ts[0:t]/1e6

        # Update title with current variable values
        fig.layout.title = 'Time (s): '+str((t-1)*dt)+', z (m): '+str(np.round(z_parcel_array[t-1], decimals=2))+', RH (%): '+str(np.round(RH_parcel_array[t-1]*100, decimals=3))+', T (K): '+str(np.round(T_parcel_array[t-1], decimals=2))
