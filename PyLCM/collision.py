...
# ---------- Module‑level constants (moved out of inner loops for speed) ----------
PI = math.pi
RHO_LIQ_DEFAULT = 1000.0
RHO_ICE_DEFAULT = 917.0
RHO_PARCEL_DEFAULT = 1.0
G = 9.81
MUELQ = 1.8325e-5
V_PARCEL_DEFAULT = 1.0
COLLECTION_KERNEL_MICRO = 'hall'

# Static lookup tables for the Hall (1980) collision efficiency
...
def collection(dt, particles_list, rho_parcel, rho_liq, p_env, T_parcel, acc_ts, aut_ts, precip_ts, sedi_removal, z_parcel, max_z, w_parcel, switch_lsm):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        if switch_lsm:
            ...
            # Submit only one task per leading‑index
            def process_collisions_for_index(i: int):
                for j in range(i + 1, len(particle_list1)):
                    process_collision(particle_list1[i], particle_list1[j])
            futures = [executor.submit(process_collisions_for_index, i)
                       for i in range(len(particle_list1) - 1)]
...
def determine_collision(dt, particle1, particle2, rho_parcel, rho_liq, p_env, T_parcel, half_length,nptcl, switch_lsm):
    ...
    R_n = (particle1.M / particle1.A / (4.0 / 3.0 * PI * RHO_LIQ_DEFAULT)) ** 0.33333333333
    R_m = (particle2.M / particle2.A / (4.0 / 3.0 * PI * RHO_LIQ_DEFAULT)) ** 0.33333333333
    
    v_r1 = ws_drops_beard(R_n, RHO_PARCEL_DEFAULT, RHO_LIQ_DEFAULT, p_env, T_parcel)
    v_r2 = ws_drops_beard(R_m, RHO_PARCEL_DEFAULT, RHO_LIQ_DEFAULT, p_env, T_parcel)
    
    v_r = abs(v_r1 - v_r2)

    if COLLECTION_KERNEL_MICRO.strip() == 'hall':
        K = PI * (R_m + R_n) ** 2 * v_r * E_H80(R_m, R_n) * E_S09(R_m, R_n, v_r, RHO_LIQ_DEFAULT,T_parcel)

    p_crit = max(particle1.A, particle2.A) * K / V_PARCEL_DEFAULT * dt
    if switch_lsm:
...
def E_H80(r1: float, r2: float) -> float:
    """Collision efficiency (Hall 1980) with LRU cache."""
    rmax = max(r1, r2)
    # Table index for radius (micron)
    if rmax * 1e6 >= R0_HALL[-1]:
        ir = 15
    else:
        ir = next(k for k, rv in enumerate(R0_HALL, start=1) if rmax * 1e6 < rv)
    ...
    if ir < 15:
        if ir >= 1:
            pp = (rmax * 1e6 - R0_HALL[ir-1]) / (R0_HALL[ir] - R0_HALL[ir-1])
