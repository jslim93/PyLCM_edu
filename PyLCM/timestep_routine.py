...
################################
    ...
    for t in range(nt):
        ...
        # Collisional Growth
        if do_collision:
            particles_list, acc_ts[t+1], aut_ts[t+1],precip_ts[t+1] = collection(dt, particles_list,rho_parcel, rho_liq, P_parcel, T_parcel, acc_ts[t+1], aut_ts[t+1],precip_ts[t+1], switch_sedi_removal, z_parcel, max_z, w_parcel, switch_lsm)
            
            # Convert mass output to per mass per sec.
            acc_ts[t+1]  = 1e3 * acc_ts[t+1] / air_mass_parcel / dt
            aut_ts[t+1]  = 1e3 * aut_ts[t+1] / air_mass_parcel / dt
        # Analysis
        spectra_arr[t+1],qa_ts[t+1], qc_ts[t+1],qr_ts[t+1], na_ts[t+1], nc_ts[t+1], nr_ts[t+1], particles_array[t+1], rc_liq_avg_array[t+1], rc_liq_std_array[t+1] = ts_analysis(particles_list,air_mass_parcel,rm_spec, n_bins, n_particles)
